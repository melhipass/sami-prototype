...
export function SettingsScreen({
  ...
  return (
    <div className="absolute inset-0 bg-black z-30 flex flex-col">
      ...
      {/* Browser Viewer — Internet Not Enabled popup */}
      {showBVNoInternetPopup && (
        <div className="absolute inset-0 bg-black/70 z-50 flex items-center justify-center">
          <div className="bg-gray-800 rounded-lg w-[520px] overflow-hidden border border-gray-700">
            <div className="px-8 pt-6 pb-2">
              <h2 className="text-white text-xl font-semibold">Internet not enabled</h2>
            </div>
            <div className="px-8 py-5">
              <p className="text-gray-300 text-base">Enable Internet Viewing before setting up browser access.</p>
            </div>
            <div className="border-t border-gray-700 flex">
              <button
                onClick={() => setShowBVNoInternetPopup(false)}
                className="flex-1 text-lg py-4 hover:bg-gray-700 transition-colors text-center font-semibold"
                style={{ color: SETTINGS_ACCENT_COLOR }}
              >Ok</button>
            </div>
      ...
      {/* Browser Access Settings Screen */}
      {showBVScreen && (
        <div className="absolute inset-0 bg-black z-50 flex flex-col" style={{ animation: 'slideInRight 0.3s ease-out forwards' }}>
          {/* Header */}
          <div className="bg-black py-4 flex items-center justify-between flex-shrink-0">
            <button
              onClick={() => setShowBVScreen(false)}
              className="ml-6 flex items-center gap-2 px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-white transition-colors"
            >
              <ChevronLeft className="w-5 h-5" />
              <span>Camera Settings</span>
            </button>
            <span className="text-white text-lg font-medium">Browser Access Settings</span>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4">
            ...
            <p className="text-gray-400 text-sm leading-relaxed mb-6">
              Browser Access enables viewing the live video stream using a web browser. Note that Internet Access must also be enabled.
            </p>
            <p className="text-gray-400 text-sm leading-relaxed mb-6">
              <span style={{ color: '#FFC7BD' }}>WARNING: Browser viewers can always listen to the live audio when access is enabled.</span>
            </p>

            {/* Enable Browser Access toggle */}
            <div className="flex items-center justify-between py-4 border-b border-gray-700">
              <span className="text-white text-base">Enable Browser Access</span>
              <button
                onClick={() => setBvEnableBrowserAccess(v => !v)}
                className={`w-14 h-8 rounded-full transition-colors relative ${!bvEnableBrowserAccess ? 'bg-gray-400' : ''}`}
                style={bvEnableBrowserAccess ? { backgroundColor: SETTINGS_ACCENT_COLOR } : {}}
              >
                <div className={`absolute top-1 w-6 h-6 bg-white rounded-full transition-transform ${bvEnableBrowserAccess ? 'right-1' : 'left-1'}`} />
              </button>
            </div>

            {/* Fields — only when browser access is enabled */}
            {bvEnableBrowserAccess && (
              <div className="mt-2 space-y-5">
                {/* Internet Name */}
                <div>
                  <label className="text-gray-400 text-sm block mb-2">Internet Name</label>
                  <div className="flex items-center gap-2">
                    <input
                      type="text"
                      value={bvNameDraft}
                      ...
                      onBlur={handleBvNameBlur}
                      placeholder="e.g. mynursery"
                      className="flex-1 px-4 py-3 rounded-lg bg-gray-900 text-white border border-gray-600 focus:border-[#5A8BBF] focus:outline-none text-base"
                    />
                    <span className="text-gray-400 text-sm whitespace-nowrap">https://samialert.net/</span>
                  </div>
                  <p className="text-gray-600 text-xs mt-1 text-right">{bvNameDraft.length}/30</p>
                  {bvInternetName && (
                    <p className="text-[#BFE3D9] text-xs mt-1">Saved: {bvInternetName}</p>
                  )}
                </div>

                {/* Browser Username */}
                <div>
                  <label className="text-gray-400 text-sm block mb-2">Browser Username</label>
                  <input
                    type="text"
                    value={bvUsernameDraft}
                    onChange={e => {
                      const val = e.target.value.slice(0, 30);
                      if (!val || BV_FIELD_ALLOWED.test(val)) setBvUsernameDraft(val);
                    }}
                    onBlur={handleBvUsernameBlur}
                  ...
                  />
                  <p className="text-gray-600 text-xs mt-1 text-right">{bvUsernameDraft.length}/30</p>
                </div>

                {/* Browser Password */}
                <div>
                  <label className="text-gray-400 text-sm block mb-2">Browser Password</label>
                  <div className="relative">
                    <input
                      type={bvShowPassword ? 'text' : 'password'}
                      value={bvPasswordDraft}
                      onChange={e => {
                        const val = e.target.value.slice(0, 30);
                        if (!val || BV_FIELD_ALLOWED.test(val)) setBvPasswordDraft(val);
                      }}
                      onBlur={handleBvPasswordBlur}
                      placeholder="Password (min 8 characters)"
                      className="w-full px-4 py-3 pr-12 rounded-lg bg-gray-900 text-white border border-gray-600 focus:border-[#5A8BBF] focus:outline-none text-base"
                    />
                    <button
                      onClick={() => setBvShowPassword(v => !v)}
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white transition-colors"
                    >
                      {bvShowPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                    </button>
                  </div>
                  <p className="text-gray-600 text-xs mt-1 text-right">{bvPasswordDraft.length}/30</p>
                </div>
              </div>
            )}
          </div>

          {/* Browser Viewer popups — sit above the BV screen */}
          {bvPopup === 'name-too-short' && (
            <div className="absolute inset-0 bg-black/70 z-[60] flex items-center justify-center">
              <div className="bg-gray-800 rounded-lg w-[520px] overflow-hidden border border-gray-700">
                <div className="px-8 py-5">
                  <p className="text-gray-300 text-base">Camera Internet Name must be between 4 and 30 characters long.</p>
                </div>
                <div className="border-t border-gray-700 flex">
                  <button onClick={() => setBvPopup(null)} className="flex-1 text-lg py-4 hover:bg-gray-700 transition-colors text-center font-semibold" style={{ color: SETTINGS_ACCENT_COLOR }}>Ok</button>
                </div>
              </div>
            </div>
          )}
          {bvPopup === 'name-in-use' && (
            <div className="absolute inset-0 bg-black/70 z-[60] flex items-center justify-center">
              <div className="bg-gray-800 rounded-lg w-[520px] overflow-hidden border border-gray-700">
                <div className="px-8 pt-6 pb-2">
                  <h2 className="text-white text-xl font-semibold">Name in use</h2>
                </div>
                <div className="px-8 py-5">
                  <p className="text-gray-300 text-base">This Internet Name is already in use. Choose another.</p>
                </div>
                <div className="border-t border-gray-700 flex">
                  <button onClick={() => setBvPopup(null)} className="flex-1 text-lg py-4 hover:bg-gray-700 transition-colors text-center font-semibold" style={{ color: SETTINGS_ACCENT_COLOR }}>Ok</button>
                </div>
          ...
          {bvPopup === 'network-problem' && (
            <div className="absolute inset-0 bg-black/70 z-[60] flex items-center justify-center">
              <div className="bg-gray-800 rounded-lg w-[520px] overflow-hidden border border-gray-700">
                <div className="px-8 pt-6 pb-2">
                  <h2 className="text-white text-xl font-semibold">Network problem</h2>
                </div>
                <div className="px-8 py-5">
                  <p className="text-gray-300 text-base">Setting the Internet Name requires an internet connection.</p>
                </div>
                <div className="border-t border-gray-700 flex">
                  <button onClick={() => setBvPopup(null)} className="flex-1 text-lg py-4 hover:bg-gray-700 transition-colors text-center font-semibold border-r border-gray-700" style={{ color: SETTINGS_ACCENT_COLOR }}>Ok</button>
                  <button onClick={handleBvNameRetry} className="flex-1 text-lg py-4 hover:bg-gray-700 transition-colors text-center font-semibold" style={{ color: SETTINGS_ACCENT_COLOR }}>Try Again</button>
                </div>
              </div>
            </div>
          )}
          {bvPopup === 'password-too-short' && (
            <div className="absolute inset-0 bg-black/70 z-[60] flex items-center justify-center">
              <div className="bg-gray-800 rounded-lg w-[520px] overflow-hidden border border-gray-700">
                <div className="px-8 py-5">
                  <p className="text-gray-300 text-base">Password must be between 8 and 30 characters long.</p>
                </div>
                <div className="border-t border-gray-700 flex">
                  <button onClick={() => setBvPopup(null)} className="flex-1 text-lg py-4 hover:bg-gray-700 transition-colors text-center font-semibold" style={{ color: SETTINGS_ACCENT_COLOR }}>Ok</button>
                </div>
